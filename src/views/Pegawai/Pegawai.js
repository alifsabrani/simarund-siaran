...
class Pegawai extends Component {
	...
	deleteItem(row) {
		Auth.fetch('http://localhost:8000/api/pegawai/' + row.join(','), {
			method: 'DELETE'
		}).then(response => {
		...
		return (
			<div className="animated fadeIn textCenter">
				...
				<Card>
					<CardHeader>
						<h4>
							<i className="fa fa-group " /> Pegawai
						</h4>
					</CardHeader>
					<CardBlock className="card-body">
						{this.state.data === undefined ? (
							...
							<BootstrapTable
								...
								search
								searchPlaceholder="Cari Nama, Jabatan, atau Alias Pegawai"
								deleteRow
								exportCSV={true}
								pagination>
								<TableHeaderColumn
...
export default withAuth(Pegawai);
