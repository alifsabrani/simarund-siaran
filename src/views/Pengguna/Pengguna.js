...
class Pengguna extends Component {
	...
	getItem(handleSuccess) {
		Auth.fetch('http://localhost:8000/api/pengguna/', {
			method: 'GET',
			timeout: 5000
		})
			.then(response => {
	...
	updateItem() {
		...
		Auth.fetch(
			'http://localhost:8000/api/pengguna/' +
	...
	deleteItem(row) {
		Auth.fetch('http://localhost:8000/api/pengguna/' + row.join(','), {
			method: 'DELETE'
		}).then(response => {
		...
		return (
			<Col md="12">
				<Row>
					<Col xs="12" md="6">
						<ButtonToolbar>
							{props.components.insertBtn} {props.components.deleteBtn}
						</ButtonToolbar>
					</Col>
					<Col xs="12" md="6">
		...
		return (
			<div className="animated fadeIn textCenter">
				...
				<Card>
					<CardHeader>
						<h4>
							<i className="fa fa-user " /> Pengguna
						</h4>
					</CardHeader>
					<CardBlock className="card-body">
						{this.state.data === undefined ? (
							...
							<BootstrapTable
								...
								search
								searchPlaceholder="Cari Username atau Level pengguna"
								deleteRow
								exportCSV={true}
								pagination>
								<TableHeaderColumn
