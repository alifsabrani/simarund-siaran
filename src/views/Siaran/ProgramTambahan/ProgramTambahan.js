...
class ProgramTambahan extends Component {
	constructor(props) {
		super(props);
	...
	getItem(tanggal, handleSuccess) {
		...
		Auth.fetch('http://localhost:8000/api/siaran/tambahan/' + tanggal, {
			method: 'GET',
			timeout: 5000
		})
			.then(response => {
					...
					<Col xs="12" md="6">
						<ButtonToolbar>
							...
							<Button
								color="danger"
								href={
									'http://localhost:8000/api/siaran/download/' +
									this.state.data.id
								}
								outline>
			...
			<div className="animated fadeIn textCenter">
				...
				<Card>
					...
					<CardBlock className="card-body">
						...
						) : this.state.data.kategori.siaran_lokal == 0 ? (
							<BootstrapTable
								height={400}
								scrollTop={'Top'}
								ref="table"
...
export default withAuth(ProgramTambahan);
