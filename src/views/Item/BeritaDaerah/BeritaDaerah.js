...
// import 'C:/www/simarund/node_modules/sweetalert/dist/sweetalert.css';
...
class BeritaDaerah extends Component {
	constructor(props) {
		super(props);
		...
		this.getBeritaDaerah = this.getBeritaDaerah.bind(this);
		this.getAliasPegawai = this.getAliasPegawai.bind(this);
		this.getNamaKategori = this.getNamaKategori.bind(this);
		this.insertBeritaDaerah = this.insertBeritaDaerah.bind(this);
		this.deleteBeritaDaerah = this.deleteBeritaDaerah.bind(this);
		this.updateBeritaDaerah = this.updateBeritaDaerah.bind(this);
	...
	componentDidMount() {
		this.getBeritaDaerah();
		this.getAliasPegawai();
		this.getNamaKategori();
	}

	getBeritaDaerah(handleSuccess) {
		Auth.fetch('http://localhost:8000/api/berita_daerah/', {
			method: 'GET',
			timeout: 5000
		})
			.then(response => {
	...
	getNamaKategori() {
		Auth.fetch('http://localhost:8000/api/kategori/nama_siaran_lokal', {
			method: 'GET'
		})
			.then(response => {
				return response.json();
	...
	updateBeritaDaerah() {
		...
		Auth.fetch(
			'http://localhost:8000/api/berita_daerah/' +
	...
	deleteBeritaDaerah(row) {
		Auth.fetch('http://localhost:8000/api/berita_daerah/' + row.join(','), {
			method: 'DELETE'
		}).then(response => {
	...
	render() {
		const options = {
			afterInsertRow: this.insertBeritaDaerah,
			...
			afterDeleteRow: this.deleteBeritaDaerah,
		...
		const saveConfirmModal = (
			<SweetAlert
				...
				onConfirm={this.updateBeritaDaerah}
				onCancel={() => {
						...
						<Col xs="12" md="9">
							<Input
								...
								onChange={e => {
									...
									Auth.fetch(
										'http://localhost:8000/api/jadwal/siaran_lokal',
										{
											method: 'GET'
										}
									)
										...
										.then(data => {
											...
											if (
												day + 1 !=
												this.state.selectedCategoryDay
											) {
												this.setState({
													moveError:
		...
		return (
			<div className="animated fadeIn textCenter">
				...
				<Card>
					<CardHeader>
						<h4>
							<i className="fa fa-film " /> Berita Daerah
						</h4>
					</CardHeader>
					<CardBlock className="card-body">
						...
						this.state.kategori === undefined ? (
							...
							<BootstrapTable
								...
								<TableHeaderColumn
									dataField="penterjemah"
									dataSort
									dataFormat={(cell, row) => {
										return cell.toUpperCase();
									}}
									csvHeader="Penterjemah"
									editable={{
										type: 'select',
										options: {
											values: this.state.pegawai
...
export default withAuth(BeritaDaerah);
