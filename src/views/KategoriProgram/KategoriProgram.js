...
class KategoriProgram extends Component {
	constructor(props) {
		super(props);
	...
	getItem(handleSuccess) {
		Auth.fetch('http://localhost:8000/api/kategori/', {
			method: 'GET',
			timeout: 5000
		})
			.then(response => {
	...
	deleteItem(row) {
		Auth.fetch('http://localhost:8000/api/kategori/' + row.join(','), {
			method: 'DELETE'
		}).then(response => {
		...
		return (
			<div className="animated fadeIn textCenter">
				...
				<Card>
					<CardHeader>
						<h4>
							<i className="fa fa-tags " /> Kategori Program
						</h4>
					</CardHeader>
...
export default withAuth(KategoriProgram);
